...
#roots are all in E
#at least one root is not in [-2,2]
import numpy as np
N=1.28
m1=N+1/N
m2=N-1/N
M1=int(np.floor(4*m1))
M2=int(np.floor(6*m1**2))
M3=int(np.floor(4*m1**3))
M4=int(np.floor(m1**4))
for a in range(-M1,1):
        ...
        for c in range(-M3,M3+1):
            for d in range(-M4,M4+1):
                ...
                if p(2)!=0 and p(1)!=0 and p(0)!=0 and p(-1)!=0 and p(-2)!=0:
                    ...
                    r3=root[2]
                    r4=root[3]
                    if not (np.isrealobj([r1,r2,r3,r4]) and [-2,-2,-2,-2]<[r1,r2,r3,r4]<[2,2,2,2]):
                        r11=r1.real
                        r12=r1.imag
                        r21=r2.real
                        r22=r2.imag
